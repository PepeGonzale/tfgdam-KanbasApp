<template>
  <Default>
    <div class="bg-blue-primary h-screen">
      ...
      <div
        class="flex-1 w-3/5 mx-auto mt-12"
      >
        <form
          class="mx-auto hidden lg:flex bg-white border-4 -space-x-px rounded-md flex-1 shadow-sm items-center relative"
          @input = "handleSearch"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            ...
            width="20"
            height="20"
            class="icon opacity-50 left-2 z-10 absolute pointer-events-none"
          >
            <path
              stroke-linecap="round"
              stroke-linejoin="round"
          ...
          <input
            ...
            type="text"
            class="bg-white rounded-md font-medium border-gray-300 text-sm w-full p-2 pl-10 text-gray-500 z-0 inline-flex items-center"
      ...
      <div class="flex-1 w-3/5 mx-auto mt-12">
        ...
        >
          <li
            class="justify-between border-b-2 items-center border-gray-500 border-2 flex rounded-md bg-gray-200 p-3 text-black text-center"
          >
                ...
                >
                  <div class="grid relative">
                    <span
                      ...
                      class="p-2 hover:bg-green-400 cursor-pointer rounded-md"
                      >Restore</span
                    >

                    <span
                      @click="deleteTask(project._id)"
                      class="p-2 hover:bg-red-200 cursor-pointer rounded-md"
                      >Delete from project</span
                    >
                  </div>
                </div>
              </div>
...
onMounted(async () => {
  ...
  const res = await api
    .get(`/board/archived/${store.selectedBoard?._id}`)
    .then((res) => {
...
const restoreTask = async (taskId: string) => {
  const res = await api.post(
    `board/${store.selectedBoard?._id}/restore/${taskId}`
  );
  console.log(res);
...
const handleSearch =  async () => {
  const res = await api.get(`board/${store.selectedBoard?._id}/archived/search?archivedSearch=${archiveSearch.value}`, {
    headers: {
