<template>
  ...
  <div class="flex-justify-end">
    <div>
      <div class="p-1.5 hover:bg-red-200 rounded-full text-red-900" @click="useLayoutStore.drawerOpen = false">
      ...
      <div class="px-3 py-2 md:px-12  text-sm md:text-base">
        <form autocomplete="off" @submit.prevent = "createTask">
          ...
          <div class="flex justify-between">
            ...
            <div>
              <label class="text-gray-500 block sm:inline"> Asgined to:</label>
            </div>
            </div>
          <div class="my-8">
            <label class="block text-gray-500">Description: (Optional)</label>
            <textarea class="border border-gray-300 w-full px-4 py-3 outline-none h-32" @input="check"
                v-model="description"></textarea>
          </div>
          <div class="my-4 items-center">
              Comments: <button class="text-blue-600 underline " @click.prevent="leaveComment">Leave a comment</button>
              ...
              />
              <button @click="commentUpload" class="bg-blue-600 rounded-lg hover:bg-blue-300 shadow-md p-2 w-24 mx-2">Reply</button>
              </div>
            </div>
          <button class="bg-purple-500 text-white px-2 py-1 rounded-sm">Add Task</button>
        </form>
      </div>
    </div>
  </div>
